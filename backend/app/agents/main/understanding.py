...
- handling 必须依据完成最终目标所需的事实依赖判断，而不是依据主题词、业务对象、表面动作或之前对话采用过的处理流程判断。
- 如果当前对话、用户输入的材料或可信上下文已经足以完成目标，只需要对已有材料进行理解、总结、解释、推理、评价或提出建议，则使用 direct。
- 只有完成最终目标必须获取新的外部事实，或必须执行受控操作时，才使用 delegated；其中稳定系统资料属于知识库事实，当前记录、统计和状态属于业务事实。
- 不要因为目标涉及业务分析、销售、订单、退款、客户等对象，就假设必须读取实时业务数据；先判断已有材料是否足够。
...
- direct 仅用于不需要获取新的外部事实或执行受控操作的请求。
