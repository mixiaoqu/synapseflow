...
def decide_route(
    ...
    *,
    ...
    clarity = str(understanding.get("clarity") or "unclear")
    handling = str(understanding.get("handling") or "unsupported")
    targets = [
        str(item)
        for item in list(understanding.get("capability_ids") or [])
        if str(item) in available_sub_agent_ids
    ]
    if clarity != "clear":
        route_type = ROUTE_CLARIFY_MAIN
    elif handling == "direct":
        route_type = ROUTE_DIRECT_RESPONSE
    elif handling == "capabilities" and len(targets) > 1:
        route_type = ROUTE_MULTI_SUB_AGENT
    elif handling == "capabilities" and targets:
        route_type = ROUTE_SINGLE_SUB_AGENT
    else:
        route_type = ROUTE_UNSUPPORTED
    return {
        "route_type": route_type,
        "intent": {"goal": str(understanding.get("goal") or "").strip()},
