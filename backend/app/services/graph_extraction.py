...
_ALIAS_GUIDANCE = """aliases 用于用户问题和图谱实体匹配，不要只输出一个别名。
必须基于 name 生成用户可能使用的其他实体名称，尽量同时包含中文和英文；qualified_name/canonical_name 继续放在对应字段，不要作为 aliases 输出。
对于 camelCase、PascalCase、snake_case、kebab-case 等技术名，必须补充英文拆词形式和高置信中文意译。
对于函数、方法、组件、模块、配置、页面、接口等技术实体，应补充用户可能在问题中使用的口语化实体名称。
每个明确实体优先输出 3-8 个 aliases；如果无法高置信生成中文，也至少输出多个基于 name 的英文拆词形式。
例如：
- UserProfile 可补充 aliases: ["user profile", "用户资料"]
- PaymentConfig 可补充 aliases: ["payment config", "支付配置"]
- OrderService 可补充 aliases: ["order service", "订单服务"]
- batchUpdateUserData 可补充 aliases: ["batch update user data", "update user data", "批量更新用户数据", "更新用户数据"]
- parseAddress.js::batchUpdateUserData 可补充 aliases: ["parseAddress.js batchUpdateUserData", "parse address batch update user data", "地址解析批量更新用户数据"]
优先输出高置信、单义、可直接指向该实体的别名；不要写过宽泛、无法明确指向该实体的词，也不要写其他独立实体名。"""
...
def _split_code_words(value: str) -> tuple[str, ...]:
    cleaned = clean_graph_text(value)
    if not cleaned:
        return ()
    ...
    for part in normalized.split():
        matches = _CODE_SPLIT_PATTERN.findall(part)
        if matches:
...
def _build_code_search_aliases(
    *,
    name: str,
    canonical_name: str | None,
) -> tuple[str, ...]:
    aliases: list[str] = []
    action_words = {"get", "set", "add", "update", "delete", "remove", "create"}
    ...
    name_words = _split_code_words(name)
    if len(name_words) >= 2:
        add(" ".join(name_words))
    for index, word in enumerate(name_words):
        if len(name_words) - index >= 2 and word in action_words:
            add(" ".join(name_words[index:]))
            break

    cleaned_canonical = clean_graph_text(canonical_name)
    if cleaned_canonical and "::" in cleaned_canonical:
        owner, symbol = cleaned_canonical.rsplit("::", 1)
        owner_name = clean_graph_text(owner.rsplit("/", 1)[-1])
        if owner_name:
            add(f"{owner_name} {symbol}")
            owner_stem = re.sub(r"\.[^.]+$", "", owner_name)
            owner_words = _split_code_words(owner_stem)
            if owner_words and name_words:
                add(" ".join([*owner_words, *name_words]))
...
5. aliases 按以下规则输出：
{_ALIAS_GUIDANCE}
   如果技术名本身更偏实现命名，但上下文能明确其产品/业务含义，请补一个用户更可能提问时使用的中文别名。
        ...
        aliases = tuple(
            ...
            for alias in dict.fromkeys(
                [
                    ...
                    *_build_code_search_aliases(name=name, canonical_name=canonical_name),
                ]
            )
