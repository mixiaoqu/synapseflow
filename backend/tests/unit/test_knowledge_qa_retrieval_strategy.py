...
def test_kb_chat_execution_plan_selects_strategy_from_question_type():
    ...
    assert relationship_plan["channels"]["vector"]["enabled"] is True
    assert relationship_plan["channels"]["graph"]["enabled"] is True
    assert summary_plan["mode"] == "parallel_fusion"
    assert summary_plan["channels"]["vector"]["enabled"] is True
...
def test_kb_chat_execution_plan_uses_multi_hop_graph_for_dependency_queries():
    ...
    call_chain_plan = build_knowledge_qa_retrieval_plan(
    ...
    assert call_chain_plan["mode"] == "graph_then_text"
...
def test_kb_chat_retrieve_text_only_runs_only_text_channel(monkeypatch):
    ...
    assert result["retrieval_result"]["metrics"]["graph_hit_count"] == 0
    assert result["retrieval_result"]["evidence_items"][0]["source"]["source_type"] == "text"
...
def test_kb_chat_retrieve_graph_only_runs_only_graph_channel(monkeypatch):
    ...
    assert calls == ["graph"]
    assert result["retrieval_result"]["status"] == "found"
    assert result["retrieval_result"]["metrics"]["text_hit_count"] == 0
    assert result["retrieval_result"]["metrics"]["graph_hit_count"] == 1
    assert (
        result["retrieval_result"]["evidence_items"][0]["source"]["source_type"]
        == "graph"
    )
