...
const navIcons: Record<(typeof adminPrimaryNav)[number]["key"] | (typeof adminNavGroups)[number]["items"][number]["key"], Component> = {
  dashboard: Odometer,
...
<template>
  <div class="flex h-screen overflow-hidden bg-slate-50">
    ...
    <aside class="flex w-[260px] shrink-0 flex-col bg-[#0F172A] text-slate-300 shadow-2xl transition-all duration-300 [&::-webkit-scrollbar]:hidden">
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-5 space-y-5" aria-label="后台主导航">
        <div class="space-y-1">
          <router-link
        ...
        >
          <button
            type="button"
            class="flex w-full items-center justify-between rounded-lg px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] transition-colors"
            :class="
              isGroupActive(group)
                ? 'text-slate-300'
          ...
          <div v-show="isGroupExpanded(group.key)" class="space-y-1">
            <router-link
