...
function roleBadge(role: string) {
  ...
  const c = ROLE_COLORS[role] ?? ROLE_COLORS.end_user;
  return { label: labels[role] ?? role, ...c };
}
...
export default function AdminLayout({
  ...
  const loginHref = `/login?next=${encodeURIComponent(pathname || "/admin")}`;
  ...
  const roleBadgeData = roleBadge(currentUser.role);
      ...
      <aside className="flex h-full w-72 shrink-0 min-h-0 flex-col border-r border-white/10 bg-[#0f1115]">
        ...
        <div className="shrink-0 border-b border-white/10 px-5 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-semibold text-white leading-tight">知识库管理后台</p>
              <p className="text-[11px] text-gray-500 mt-0.5">Knowledge Ops Console</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-0.5">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`h-4 w-4 shrink-0 transition-colors ${active ? "text-gray-700" : "text-gray-500 group-hover:text-gray-300"}`} />
        ...
        <div className="shrink-0 border-t border-white/10 p-4">
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
            <div className="flex items-center gap-3">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-semibold ${avatarPalette(currentUser.id)}`}>
                {initials}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <span className={`inline-block mt-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${roleBadgeData.bg} ${roleBadgeData.text}`}>
                  {roleBadgeData.label}
                </span>
              </div>
            </div>
            <div className="mt-4 flex gap-2">
              <Link
                href="/ask"
                className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/10 px-3 py-2 text-xs text-gray-300 transition-colors hover:bg-white/8 hover:text-white"
              >
                前台问答
              </Link>
              <button
